...
**Source:** private pre-v0.1 schema-v4 process-only candidate plus incident-centric browser-history/detail UI and bounded IPC-worker lifetime hardening head `97cc45f`; policy-version `0.3.0` preserves exact process TERM/KILL eligibility and disables runtime-artifact admission in every embedded pack; local focused Swift/bundle/isolated-smoke verification and the exact-head full CI gate pass

**Remote:** bounded IPC-worker head `97cc45f9ccdbd5990216dfd36683ec28c7ee0cb3` passed exact-head private macOS `backend` run `33640035615`; memory-verification truth head `7455b7717c947e834507b0de52faa3884777e47a` passed run `33640510272`. Both include formatting, strict clippy, workspace tests, release build, native frontend tests and bundle. The installed generation-15 daemon remains sourced from process-only implementation head `17cb6a5035f979dc1849c7165038e365d8a202e1`
...
- a direct AppKit `@main` that strongly retains the single delegate for the blocking App run loop, with AppKit-owned `NSStatusItem`/`NSPopover` and reusable ordinary-window lifecycles around one shared SwiftUI state/router and one shared `340 × 420` content size; the two long-lived hosts currently bind separate `NavigationStack` instances to the same router path, which is now the leading unproved source hypothesis for the history/Accessibility update storm and must be retired or falsified before installed-App reuse;
...
Later the same day, menu-bar diagnosis activated the ordinary browser-history window and requested the App's Accessibility tree. The owner then observed the App at approximately 8 GB and terminated it. A retained macOS CPU-resource diagnostic independently captured the same PID increasing physical footprint from 106.89 MB to 1845.11 MB in 86.99 sampled seconds while averaging 57% CPU. Its heaviest sampled path is SwiftUI/AttributeGraph transaction flushing, including `LazySubviewPlacements`, `ForEachState`, and accessibility-property application; it does not implicate the bounded socket-worker queue. This falsifies continued installed-App dogfood acceptance and narrows the incident to a SwiftUI history/navigation/Accessibility update storm, but does not yet prove which source-level feedback edge causes it. The two long-lived hosts' shared `AppRouter.path` binding is the leading architecture hypothesis. The App remains stopped; no installed relaunch, menu-host manipulation, or Accessibility stress is authorized before a source repair and a separately bounded fixture-first reproduction with a hard resource cutoff. Generation 15 remained independently healthy `ReadyEnforce` after the App stopped.
...
- source repair and deterministic regression coverage for the SwiftUI history/navigation/Accessibility update storm, followed only with owner authorization by a bounded fixture-first reproduction with a hard resource cutoff; ordinary installed-App dogfood remains stopped;
