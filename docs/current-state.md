...
**Activation:** process-only enforcement is active on generation 15 after exact-head CI, candidate-A install/restart/real rollback to generation 13, candidate-B reinstall/restart/accept, a full-timing managed process-only field pass, final report-only containment, and a later stable post-arm sweep. The schema-v4 packaged App was replaced with the bounded IPC-worker build from current head `7455b77` and passed a 20-minute externally sampled ordinary-polling memory soak, but a later browser-history/Accessibility interaction produced a second severe memory runaway. The owner terminated the App; it remains intentionally stopped and is not accepted for continued installed use. Process eligibility remains limited to exact admitted CfT `151.0.7922.34`; newer observed CfT `152.0.7977.42` sessions remain `PROTECTED`. Runtime-artifact eligibility is disabled.
...
- stored browser overview and incident-grouped history presentation rebuilt once per state transition and published only when its value changes, direct stable-identity `ForEach` rendering, and one outer Accessibility element per history link rather than nested row/link semantics;
...
- the current source repair adds focused router-host isolation and cached-history projection tests plus a 50-incident fixture. Its final exact-source real-Accessibility gate launched only an owned fixture App, held one exact native window element for 120 seconds, completed 462 child-tree traversals with zero read misses, and sampled RSS externally at 70,992 KiB initial/maximum and 19,712 KiB final under a 384 MiB absolute / 96 MiB retained-growth cutoff. It did not contact the installed daemon;
...
- owner-authorized replacement and bounded installed-App field acceptance for the repaired frontend; the older installed App and ordinary App dogfood remain stopped;
