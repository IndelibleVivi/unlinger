...
Current verification: `cargo fmt --all -- --check`, strict workspace clippy, and all 33 Rust tests pass. The suite now includes a strict red/green regression for detached crashpad helpers and deterministic terminal completion timestamps for successful and post-signal-failure receipts. The exact macOS signal integration still targets only an isolated child created by the test.
...
After the fixes, a fresh native doctor inspected 384/384 current-user processes with zero unreadable entries in 62 ms, and the final report-only dry-run found no incident. No daemon or field browser remains active.
