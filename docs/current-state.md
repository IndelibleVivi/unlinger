...
**Activation:** process-only enforcement is active on generation 15 after exact-head CI, candidate-A install/restart/real rollback to generation 13, candidate-B reinstall/restart/accept, a full-timing managed process-only field pass, final report-only containment, and a later stable post-arm sweep. The schema-v4 packaged App was replaced with the bounded IPC-worker build from current head `7455b77`, launched with explicit owner authorization, and passed a 20-minute externally sampled ordinary-polling memory soak. It remains running. Process eligibility remains limited to exact admitted CfT `151.0.7922.34`; newer observed CfT `152.0.7977.42` sessions remain `PROTECTED`. Runtime-artifact eligibility is disabled.
...
On 2026-09-03 the rebuilt App was staged, verified, installed byte-for-byte from the release bundle and launched with owner authorization; the previous installed bundle was moved recoverably to Trash. One external 20-minute soak sampled RSS every five seconds and physical footprint approximately every minute, with a 512 MiB automatic UI-only termination threshold. The same App PID survived throughout. RSS finished at 9.5 MiB and peaked at 24.6 MiB; sampled physical footprint finished at 25.1 MiB and peaked at 25.8 MiB, repeatedly declining rather than rising monotonically. The limit never approached, the installed bundle remained signature/plist/source-bundle exact, and generation 15 remained healthy `ReadyEnforce`. This accepts the rebuilt App's ordinary polling path for continued dogfood; it does not reproduce the original 9 GB event or accept repeated Computer Use/accessibility-tree stress.
...
- longer ordinary App dogfood and, only if deliberately needed, a separately bounded reproduction of Computer Use/accessibility-tree stress; the 20-minute ordinary-polling soak does not prove the original approximately 9 GB event's exact cause;
