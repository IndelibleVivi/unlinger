...
Pause/resume, named retry, exact protection changes, and service lifecycle mutations advance the in-memory cleanup-policy revision. A cycle must observe the same revision and enforcement epoch again immediately before opening a cleanup attempt and before every signal. In particular, a retry acknowledged after confirmation invalidates that cycle, clears only the named block/cooling candidate, and requires fresh cooling; it is not an immediate resend authorization.

Native process-exit, wake, and memory-pressure events are bounded scheduling hints. They never authorize cleanup or weaken the age, version, cooling, identity, or protection gates, and a failed event source falls back to periodic reconciliation. The product has no admitted threshold for notifying on an ambiguous incident under sustained pressure; an aggregate ambiguous count is not notification authority.
