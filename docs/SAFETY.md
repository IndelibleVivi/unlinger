...
Accepted generation 17 and the schema-v5 App are installed and loaded for private dogfood. Generation 17 actively enforces the `0.4.0` process-only policy after exact-head CI, candidate-specific rollback/reinstall, packaged App/live-socket checks, explicit acceptance and arm, and a stable later sweep. Direct `unlingerd` invocation still defaults to report-only.

This establishes generation-17 installation, launchd ownership, SQLite-v7 migration/rollback, persistent reconciliation, schema-v5 App integration, protected-session coexistence, and current process-only activation on one host. Generations 9 and 13 passed historical process/artifact runs; generation 15 separately passed the CfT-151 process-only programme with zero artifact candidates and zero receipt/journal artifact actions. Generation 17 has not run that signal harness and cannot borrow its result for CfT 152. None of this establishes live wake/pressure, sleep survival, an ambient ordinary eligible-incident cleanup, multi-day zero-false-positive, broad family/version support, signing, or release readiness.

Generation 17 is accepted and has no pending rollback lease. Candidate A generation 16 began report-only, retained generation 15 and its SQLite-v6 snapshot through a fresh acceptance-scoped lease, passed exact report-only restart, and executed real rollback to healthy generation 15 under the old CLI/daemon. Candidate B reinstalled the same exact source as generation 17, repeated readiness/restart/schema-v5 App checks, and was accepted before activation. `DatabaseBackedUp` remains a conservative restore boundary, durable `RollbackInProgress` makes physical rollback replayable across candidate/prior/mixed selection cuts, and `AcceptanceInProgress` still recovers to the prior generation; only durable `Accepted` retires rollback material.
