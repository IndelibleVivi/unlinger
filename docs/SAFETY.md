...
The ordinary daemon default remains report-only. The maintainer's accepted reference installation is generation 32: Playwright `0.6.0`, other packs `0.4.0`, SQLite v10, frontend schema v5 and the `c272942` per-candidate clone gate. Generation 31 proved install/restart and real rollback to generation 30; the exact artifact was then freshly installed, accepted and explicitly armed as healthy `ReadyEnforce` generation 32. Its first two stable clone observations made no mutation because the live `.app.bundle` main lacked generic bundle facts; the narrow source correction is not installed yet. [Current state](current-state.md) owns the exact generation, activation and controlled-cleanup evidence. Installation and a healthy sweep do not establish successful clone cleanup, multi-day safety, an ordinary ambient eligible process cleanup, broad family/version support or release readiness.
