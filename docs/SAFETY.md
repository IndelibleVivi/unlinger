...
The ordinary daemon default remains report-only. The maintainer's accepted reference installation is generation 25: Playwright `0.6.0`, other packs `0.4.0`, SQLite v10 and frontend schema v5, explicitly activated in process-only enforcement after its own report-only install/restart/real-rollback/fresh-reinstall proof. [Current state](current-state.md) owns the exact generation, activation and controlled-cleanup evidence. Installation and a healthy sweep do not establish multi-day safety, an ordinary ambient eligible cleanup, broad family/version support or release readiness.
