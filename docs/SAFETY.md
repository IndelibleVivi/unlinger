...
The schema-v3/SQLite-v6 candidate has an acceptance-scoped rollback lease. Candidate install is report-only only; `CandidateReadyReportOnly` retains the prior manifest/plist/v5 database, blocks install/uninstall/mode changes, and permits only exact report-only restart plus explicit accept or rollback. `AcceptanceInProgress` still recovers to the prior generation; only durable `Accepted` retires rollback material. The installed proof passed: a real rollback restored the v5 store and the exact generation-9 binary opened it at healthy ReadyReportOnly before generation 12 was installed. The generation-12 lease remains pending during initial dogfood.
