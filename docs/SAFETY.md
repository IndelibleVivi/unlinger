...
Deletion is a distinct enforce-only gate inside that canonical path. The exact candidate identity set must remain unchanged across two consecutive 15-minute storage observations. A complete canonical native process snapshot must prove that no process executable or absolute argv path lies within a candidate and no bundle-confirmed clone-cleanup helper carries that candidate's exact six-character suffix. Ordinary Chrome main, renderer, GPU and utility processes outside the candidates do not block; a valid unrelated helper suffix also does not block the current set. A Chrome-looking process with insufficient bundle/path/argv facts, or a clone-cleanup helper with a missing, malformed or ambiguous suffix, fails closed. Report-only, pause, drain, startup/recovery, non-ready state, concurrent scan/cleanup state, incomplete process coverage, candidate change or unexpected filesystem shape prevents mutation. Eligible deletion is descriptor-relative and no-follow, stays beneath scanner-validated candidate directories, rechecks candidate identity and immediately rescans before persisting public state. Failure retains detected or unavailable truth rather than inventing clear. Raw root/child paths never enter SQLite, IPC, logs or public DTOs; only the previous candidate identities are retained in memory, so daemon restart restarts the stability window. This boundary protects ordinary user-computer concurrency; it does not claim a hostile same-UID race-resistant filesystem sandbox. This corrected source gate is not installed or field-verified.
...
The ordinary daemon default remains report-only. The maintainer's accepted reference installation is generation 27: Playwright `0.6.0`, other packs `0.4.0`, SQLite v10 and frontend schema v5. It was previously activated in process-only enforcement after its transactional proof, then restored to healthy `ReadyReportOnly` by the later generation-28 rollback. [Current state](current-state.md) owns the exact generation, activation and controlled-cleanup evidence. Installation and a healthy sweep do not establish multi-day safety, an ordinary ambient eligible cleanup, broad family/version support or release readiness.
