...
Source SQLite v11 treats cleanup impact and Chrome clone cleanup results as independent durable authorities rather than reconstructing them from bounded observation history or a later smaller directory count. It retains the v9 action-attribution correction, v8 task ownership and v7 impact/span invariants, composes the v10 private optional session-owner leases, and adds one storage PREPARED row before deletion plus an atomic terminal-result/latest-observation commit. Process cleanup and its impact row retain their existing atomic recovery invariant. A storage attempt left PREPARED becomes `delivery_unknown`; recovery must succeed before another due storage cycle may reach deletion. Only measurement-complete proved process reclaim contributes memory totals, and exact cleanup event identity resolves recent process settlement even when a later failure exists. Equivalent observations coalesce into bounded spans, but neither observation age nor count retention may evict the lifetime aggregate or cleanup detail before its separate minimum window.
