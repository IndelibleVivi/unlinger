...
Deletion is a distinct enforce-only gate inside that canonical path. The exact candidate identity set must remain unchanged across two consecutive 15-minute storage observations. A complete canonical native process snapshot must prove there is no bundle-confirmed `com.google.Chrome` or Chrome Helper process, including the clone-cleanup helper; a malformed or missing helper suffix does not weaken that block. Report-only, pause, drain, startup/recovery, non-ready state, concurrent scan/cleanup state, incomplete process coverage, candidate change or unexpected filesystem shape prevents mutation. Eligible deletion is descriptor-relative and no-follow, stays beneath scanner-validated candidate directories, rechecks candidate identity and immediately rescans before persisting public state. Failure retains detected or unavailable truth rather than inventing clear. Raw root/child paths never enter SQLite, IPC, logs or public DTOs; only the previous candidate identities are retained in memory, so daemon restart restarts the stability window. This boundary protects ordinary user-computer concurrency; it does not claim a hostile same-UID race-resistant filesystem sandbox. The source path is not installed or field-verified in generation 25.
