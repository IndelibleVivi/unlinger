...
The schema-v3/SQLite-v6 pre-v0.1 source tranche does not replace or reload that service. Generation 9 can open schema v5 only, and the current installer has no acceptance-scoped rollback lease after candidate-ready. Installed v3 acceptance remains prohibited until prior manifest/plist/v5 database material survives through explicit accept/rollback and a real generation-9 binary proves it can open the restored store and return healthy ReadyReportOnly.
