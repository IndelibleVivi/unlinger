...
Accepted generation 13 and the schema-v4 App are installed and loaded for private dogfood. Generation 13 passed a controlled full-timing cleanup/restart run and a later brief re-arm, then was deliberately contained at healthy `ReadyReportOnly` because process authorization did not separately accept the DAP P2s. The current source candidate is process-only and remains uninstalled until source/CI plus transactional rollback/reinstall gates pass. Direct `unlingerd` invocation also defaults to report-only.

This establishes installation, launchd ownership, persistent reconciliation, protected-session coexistence, and historical mode-transition mechanics on one host. Generations 9 and 13 each passed one complete managed full-timing process/artifact/restart transaction; those historical artifact-enabled runs remain evidence only. The next candidate must repeat the exact process programme while proving zero artifact candidates and zero receipt/journal artifact actions. None of this establishes live wake/pressure, sleep survival, ambient ordinary eligible-incident cleanup, multi-day zero-false-positive, broad family/version support, signing, or release readiness.

Generation 13 is accepted and has no pending rollback lease. A new process-only candidate install must begin at report-only, retain generation 13 through a fresh acceptance-scoped lease, pass exact report-only restart, execute a real rollback to healthy generation 13, and then be reinstalled as a fresh generation before acceptance. `DatabaseBackedUp` remains a conservative restore boundary, durable `RollbackInProgress` makes physical rollback replayable across candidate/prior/mixed selection cuts, and `AcceptanceInProgress` still recovers to the prior generation; only durable `Accepted` retires rollback material.
