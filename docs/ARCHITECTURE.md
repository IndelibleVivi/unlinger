...
The acceptance transaction uses `AcceptanceInProgress` as a pre-linearization state: recovery restores the prior report-only generation. Durable `Accepted` is the commit point; recovery then finishes cleanup rather than rolling back. While a ready lease is pending, install, uninstall and mode changes are blocked; `restart-report-only` preserves the lease and re-proves the exact candidate runtime. This source contract still requires the real generation-9 rollback/open and installed App/restart runbook before an installed claim.
