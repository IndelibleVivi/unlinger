...
| Chrome `code_sign_clone` residue family | yes, exact current-user path shape | source: valid enforce only after the same exact candidate set spans two 15-minute observations, complete native absence of every bundle-confirmed Chrome/Chrome Helper process, and every lifecycle/filesystem gate; installed generation 25: observation only | yes; temp fixtures cover deletion plus report-only, stability reset, live/incomplete process blocks, no-follow shapes and failed-removal rescan | none for deletion; historical installed observation only | one owner-observed residue family motivated the tranche; installed detected/clear observations are not cleanup evidence | logical bytes are not a physical-reclaim guarantee; source cleanup is not installed or field-verified |
