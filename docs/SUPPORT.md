...
| Chrome `code_sign_clone` residue family | yes, exact current-user path shape | source: valid enforce only after the same exact candidate set spans two 15-minute observations, a complete native snapshot proves no matching-suffix cleanup Helper or executable/absolute-argv path references a candidate, and every lifecycle/filesystem gate passes; ordinary Chrome/Helpers outside candidates and valid unrelated suffixes do not block | yes; temp fixtures cover deletion plus report-only, stability reset, ordinary-Chrome non-blocking, exact/unrelated suffixes, candidate-path and incomplete-process blocks, no-follow shapes and failed-removal rescan | none for this corrected deletion gate | one owner-observed residue family motivated the tranche; installed detected/clear observations are not proof of the corrected cleanup gate | logical bytes are not a physical-reclaim guarantee; the corrected source gate is not installed or field-verified |
...
| Installed App/v5 integration | matching schema-v5 ad-hoc-signed private App installed and running; bundle identity/signature/plist and exact launch verified; generations 24 and 25 each passed seven live-socket tests before and after candidate restart, and rollback generation 23 passed the same seven; fresh visual UI and packaged-notification observation remain unverified |
| Current source/installed implementation | both retain schema v5/v4/v3 plus operator v1, SQLite v10, Playwright `0.6.0`, other packs `0.4.0`, exact CfT 151/152 process allowlist and disabled runtime-artifact flags; source additionally has enforce-only exact Chrome clone cleanup, while installed generation 25 remains observe-only for that storage family |
...
The task-owned lane has isolated controlled process-only evidence for exact CfT 151 and 152, plus historical installed generation-23 CfT-152 cleanup and actual App impact readback. The host-owned ordinary lane is now installed with its fixture/store/IPC/CLI and migration proof, but it still has no automatic host adapter or ambient cleanup receipt. Unregistered, unsupported or unverified controllers remain protected. Generation 15 separately retains the historical complete managed process/restart/containment point; it is not substituted for generation-25 cleanup evidence. Ordinary ambient enforcement acceptance remains false. Multi-day dogfood, broader field evidence, Intel/universal, signing/notarization/distribution and public alpha remain open. See [current-state](current-state.md) for exact outcomes and [PRE_V0_1_ACCEPTANCE](PRE_V0_1_ACCEPTANCE.md) for claim levels.
