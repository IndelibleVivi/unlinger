...
| Installed generation 17 | schema v1 operator plus v5/v4/v3 frontend endpoints; SQLite v7; accepted, healthy `ReadyEnforce`; exact armed generation/epoch; `0.4.0` process-only policy; no candidate rollback lease |
...
Generation 17 is owner-authorized and actively enforces the process-only `0.4.0` policy for exact controllerless CfT `151.0.7922.34` and `152.0.7977.42` after its own install/rollback/App/restart transaction and a later healthy sweep. The currently observed CfT 152 session is protected because controller-version authority is incomplete. Generation 15 retains the only current-policy-family controlled process-only result: one eight-process CfT-151 cleanup/restart point with zero artifact candidates/actions. Generation 17 has no candidate-specific controlled cleanup or ambient eligible-cleanup result, so ambient enforcement **acceptance** remains false. Multi-day dogfood, broader field evidence, Intel/universal, signing/notarization/distribution and public alpha remain open. See [`PRE_V0_1_ACCEPTANCE.md`](PRE_V0_1_ACCEPTANCE.md) for claim levels.
