...
| Chrome `code_sign_clone` residue family | yes, exact current-user path shape | source: valid enforce evaluates each exact candidate independently; a candidate must span two 15-minute observations, have no matching-suffix cleanup Helper or executable/absolute-argv reference, and pass every lifecycle/filesystem gate; a referenced candidate stays protected without blocking stable unreferenced siblings, while incomplete global proof blocks all | yes; temp fixtures cover full and subset deletion, report-only mixed projection, per-candidate stability, ordinary-Chrome non-blocking, exact/unrelated/malformed suffixes, executable/absolute-argv references, incomplete process proof, no-follow shapes, partial remover failure and unavailable rescan | none for this per-candidate deletion gate | one owner-observed residue family motivated the tranche; installed generation 30 observed a real live candidate reference but has not run this per-candidate source gate | logical bytes are not a physical-reclaim guarantee; the per-candidate source gate is not installed or field-verified |
...
| Current source/installed implementation | both retain schema v5/v4/v3 plus operator v1, SQLite v10, Playwright `0.6.0`, other packs `0.4.0`, exact CfT 151/152 process allowlist and disabled runtime-artifact flags; installed generation 30 contains mapped-vnode snapshot, transaction recovery and the aggregate candidate-reference clone gate and is `ReadyEnforce`; source additionally refines stability, reference protection and removal to each exact clone candidate |
