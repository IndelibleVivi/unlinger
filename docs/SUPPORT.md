...
| `playwright` | yes | yes | same exact controllerless two-point CfT allowlist and all hard gates | same protections | yes | CfT 151 historical process/artifact points on generations 9/13 plus the generation-15 process-only point; no generation-19 or CfT-152 cleanup evidence | no |
...
| Installed generation 19 | schema v1 operator plus v5/v4/v3 frontend endpoints; SQLite v7; accepted, healthy `ReadyEnforce` after the owner-authorized repair transaction on 2026-09-08; exact generation/epoch-bound authority; `0.4.0` process-only policy; no candidate rollback lease |
...
| Current source/installed line | schema v5/v4/v3 plus operator v1; SQLite v7; policy version `0.4.0`; exact CfT 151/152 process allowlist; all runtime-artifact flags false; impact/spans and observe-only storage residue |

Installed SQLite schema v7 followed a candidate-specific v6 backup/rollback/open proof. Generation 16 installed and restarted report-only, then actually rolled back to healthy generation 15; the exact old CLI/daemon reopened the restored SQLite-v6 store. The same exact candidate freshly reinstalled as generation 17, repeated schema-v5 App/live-socket checks before and after restart, and was accepted before activation. No lease remains. Historical generation-9→12 and generation-14→13 rollback/open transactions remain separate evidence.
...
Generation 17 was owner-authorized to enforce the process-only `0.4.0` policy for exact controllerless CfT `151.0.7922.34` and `152.0.7977.42` after its own install/rollback/App/restart transaction and a later healthy sweep. The 2026-09-08 audit found a terminal `Failed` instance; owner-authorized exact recovery and explicit re-arm restored healthy `ReadyEnforce`. The observer fix then passed generation-18 rollback to 17 and fresh generation-19 installation/restart/App/live-observation/acceptance/arm. Observed controller-bearing sessions remain protected because controller-version authority is incomplete. Generation 15 retains the only current-policy-family controlled process-only result: one eight-process CfT-151 cleanup/restart point with zero artifact candidates/actions. Generation 19 has no candidate-specific controlled cleanup or ambient eligible-cleanup result, so ambient enforcement **acceptance** remains false. Multi-day dogfood, broader field evidence, Intel/universal, signing/notarization/distribution and public alpha remain open. See [`PRE_V0_1_ACCEPTANCE.md`](PRE_V0_1_ACCEPTANCE.md) for claim levels.
