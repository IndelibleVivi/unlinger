...
Missing, mixed, wrong-product, or wrong-version facts produce `PROTECTED`. The source Playwright `0.6.0` pack has two disjoint exact controller fields: `task_controller_version = "1.63.0-alpha-2026-08-31"` and `session_owner_controller_version = "1.62.1"`. The task lane requires the native package/registry/socket facts plus an exact daemon-issued task binding. The optional ordinary lane retains the host's existing session name and requires a path-free registry namespace, exact host owner lifecycle and exact controller identity inside that activation/release window. In both lanes, release, complete client visibility and no active work are required in addition to every existing browser/protection gate. A lease and process agreeing on an unsupported version is still protected. Unregistered, active-owner, unsupported or otherwise unverified controllers remain protected. Agent-browser and Puppeteer remain `0.4.0` and controllerless. The installed policy is recorded separately in [current-state.md](current-state.md). See [task ownership](TASKS.md) and [operator IPC](IPC.md#optional-host-session-owner-commands).
...
All current source and installed packs use `os_term_only`, admit `ffmpeg` only through the shared exact direct-controller-child/process-group join, and set `devtools_active_port = false`. Both exact Playwright controller lanes above are installed in the generation-34 lineage; that reference service is accepted, armed and healthy `ReadyEnforce` after generation 33 proved install/restart and real rollback to generation 32. Browser versions and all artifact flags are unchanged. The ordinary process lane has installed migration/App/socket proof but no supported automatic host adapter or ordinary process-cleanup receipt yet; the separate clone-storage lane has one bounded successful result while ordinary Chrome stayed open. A plausible same-user, no-TTY recorder sharing the browser process group but not admitted into the incident protects the whole candidate rather than leaving residue behind. The analyzer produces no runtime-artifact candidate.
...
Historical field evidence includes detached ephemeral Playwright-style sessions using Chrome for Testing `151.0.7922.34`. Installed generation 23 adds two controlled task-owned CfT `152.0.7977.42` terminal cleanups, each with an eight-process tree, zero survivors, both revival checks and zero artifact actions. That evidence belongs to the exact task-controller lane; it does not accept the new ordinary host-owner lane, an ambient controller, or multi-day dogfood. Earlier managed attempts failed closed at zombie, transport, old artifact-scan, empty-argv, counterexample-proof, or read-only polling boundaries. Generations 9 and 13 later passed controlled installed runs with exact DAP removals, but those runs predate the current process-only policy and remain historical evidence, not current artifact authority or a supported version range. [Current state](current-state.md) owns the detailed field record.
