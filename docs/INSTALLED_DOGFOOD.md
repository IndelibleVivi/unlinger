...
- Inspect the exact current service before replacement. A healthy service must be quiescent; a terminal `Failed` instance is also admitted by the transactional `StableOrFailed` recovery path. Preserve its error evidence and do not treat a later scan as rehabilitation. An existing enforce service requires owner-authorized replacement; the transaction disarms/drains the exact prior instance and records a report-only rollback floor.
...
Reinstall the same exact-head candidate as a fresh generation and fresh acceptance transaction, repeat current-schema App and report-only restart checks, and retain the second candidate at `candidate_ready_report_only` unless current owner authorization already covers acceptance. Record the fresh transaction separately; the first candidate's real rollback proves the same unchanged artifact's rollback path, while the retained candidate still needs its own restart, App and healthy/quiescent checks.
