...
14. If the active pack explicitly admits an artifact, clean only artifacts whose live references are gone; current source and installed packs admit none.
...
Chrome code-sign clone residue is a separate storage family and an automatic-cleanup target in valid enforce mode; it is not a process runtime artifact and does not inherit process-signal or runtime-artifact authority. Source may observe only the exact current-user `X/com.google.Chrome.code_sign_clone` root and exact `code_sign_clone.<six ASCII alphanumeric characters>` directories containing a real `Google Chrome.app.bundle` directory (or the upstream feature-disabled `Google Chrome.app` spelling). It may report candidate count, logical regular-file size, observation time, shape status, reference-check status, cleanup eligibility and typed reason IDs. It must not persist or display the root path, clone child names or file contents. Candidate identities may live only in daemon memory for the consecutive-observation gate and are discarded on restart.

Logical size is not a reclaim promise because APFS clones may share physical blocks. The clone root and clone directory must be real current-user directories, and the app-bundle root must be a real directory. Traversal is relative to open directory descriptors, rejects symlink directory traversal, and treats descendant symlinks as leaves: their targets are never followed or included in regular-file totals. Unexpected top-level names, unsupported nodes, wrong root/clone ownership, unreadable traversal, arithmetic overflow, incomplete native process coverage or insufficient Chrome-looking process facts make cleanup fail closed.

Automatic cleanup evaluates each exact candidate identity independently across two consecutive 15-minute storage observations. It additionally requires a healthy ready unpaused and non-draining enforce daemon and a current complete canonical native process snapshot. A candidate is removable only when it was present unchanged in both observations, no process executable or absolute argv path lies within that candidate, and no bundle-confirmed Chrome `--type=code-sign-clone-cleanup` helper names its exact `--unique-temp-dir-suffix`. A referenced candidate remains protected without blocking stable unreferenced candidates in the same root. A valid unrelated suffix does not reference another candidate; a cleanup helper with a missing, malformed or ambiguous suffix, incomplete process coverage, or insufficient Chrome-looking process facts fails closed for every candidate. The narrow exception is an exact non-cleanup `Google Chrome` main executable already proved inside a scanner-admitted candidate: that path fact fully protects the candidate even when the upstream `.app.bundle` clone container cannot supply the generic `.app` bundle fact. It grants no eligibility to that candidate and does not apply to Helpers, cleanup-type arguments, argv-only references or candidate-external processes. Ordinary bundle-confirmed Google Chrome and Chrome Helper processes whose executable and absolute argv paths stay outside every candidate do not block stale-clone cleanup.

`reference_check` is an aggregate safety fact: it is `incomplete` if global proof is incomplete, `referenced` when at least one candidate is referenced, and `complete_no_references` only when all candidates are unreferenced. Consequently `reference_check: referenced` and `automatic_cleanup_eligible: true` may coexist when at least one different candidate is stable and unreferenced. Report-only performs the same projection but never mutates. A new or identity-changed candidate waits for its own next observation without resetting unchanged siblings; daemon restart discards all remembered identities. The executor receives only the eligible exact subset, reuses scanner-held no-follow descriptors, rechecks each candidate descriptor and root-entry identity, recursively unlinks only entries below that exact directory, and never follows a symlink or crosses outside it. After every attempted cleanup it immediately rescans and persists the real remaining state. A successful subset removal with protected survivors reports `storage_residue.automatic_cleanup_partial`; a complete removal reports `storage_residue.automatic_cleanup_completed`. Any removal error, incomplete rescan, or failure to prove every planned identity absent reports failure and must not present uncleared contents as clear. This contract models normal user-computer concurrency and does not claim resistance to a deliberately racing hostile same-UID process. Source completion is separate from installation, activation and field acceptance.

Each automatic removal attempt is durable before it can destroy anything and honest afterwards. The daemon persists a PREPARED attempt record before any descriptor-relative deletion and refuses to run that deletion when the record cannot be written. After the deletion and immediate rescan it commits one SQLite transaction holding both the terminal attempt result and the real latest residue observation, so a result can never disagree with the observation it settled against. The attempt row stores only an opaque internal attempt token, timestamps, the planned candidate count, before/after candidate counts and logical-byte sums, the proved removed count, the retained not-planned count, and one typed `complete | partial | failed | delivery_unknown` outcome. No pathname, candidate name, argv, user name or raw candidate identity is persisted: logical bytes are never presented as physical APFS reclaim. `complete` means every planned candidate was proved absent and no residue remains, `partial` means every planned candidate was proved absent while unplanned residue remains, and `failed` covers every other proved or unprovable removal outcome. An attempt still PREPARED after a crash or restart is finalized as `delivery_unknown` and is never rehabilitated into a success claim from a later lower directory count; the later observation is reported separately. In-process recovery of an unresolved PREPARED attempt is retried before every due storage cycle, and a recovery that cannot be made durable skips that cycle so no new deletion can start against uncertain delivery. Source frontend schema v5 exposes the latest attempt only as optional additive public facts (typed outcome, timestamps and aggregate counts/logical bytes) and never as a pathname, attempt identity or deletion command; v4 and v3 keep their existing meaning and cannot acquire it.
...
- the latest typed storage-residue observation and its current automatic-cleanup eligibility, never a pathname or App-side deletion command.
- the current storage cleanup attempt/result authority: a durable PREPARED record written before any clone-directory deletion plus one terminal result committed in the same transaction as the latest residue observation, carrying only typed outcome, timestamps and aggregate counts/logical bytes.
