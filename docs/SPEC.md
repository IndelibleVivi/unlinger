...
Logical size is not a reclaim promise because APFS clones may share physical blocks. The clone root, clone directory and app-bundle root must be real directories. Traversal is relative to open directory descriptors, rejects symlink directory traversal, and treats descendant symlinks as leaves: their targets are never followed or included in regular-file totals. Unexpected top-level names, unsupported nodes, wrong root/clone ownership, unreadable traversal, or arithmetic overflow make the observation unavailable. Shape completeness describes this no-follow observation, not deletion readiness. Reference proof is currently incomplete, so `automatic_cleanup_eligible` is always false and no source path deletes these clones. Any future quarantine/deletion work requires complete live-reference proof, recovery and race contracts, tests, a separate owner decision, and its own installed acceptance.
