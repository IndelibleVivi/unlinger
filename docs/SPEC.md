...
Automatic cleanup requires the same exact candidate identity set in two consecutive 15-minute storage observations, a healthy ready unpaused and non-draining enforce daemon, and a current complete canonical native process snapshot proving that no process executable or absolute argv path lies within a candidate and no bundle-confirmed Chrome `--type=code-sign-clone-cleanup` helper names a matching `--unique-temp-dir-suffix`. A valid unrelated suffix does not reference the current candidate set; a cleanup helper with a missing, malformed or ambiguous suffix fails closed. Ordinary bundle-confirmed Google Chrome and Chrome Helper processes whose executable and absolute argv paths stay outside every candidate do not block stale-clone cleanup. Report-only performs the same observation and may truthfully project `automatic_cleanup_eligible`, but never mutates. Candidate-set change restarts the in-memory stability window. The executor reuses the scanner-held no-follow descriptors, rechecks both the candidate descriptor and its root entry identity, recursively unlinks only entries below that exact directory and never follows a symlink or crosses outside it. After every attempted cleanup it immediately rescans and persists the real remaining state; any removal or rescan failure must not present uncleared contents as clear. This contract models normal user-computer concurrency and does not claim resistance to a deliberately racing hostile same-UID process. Source completion is separate from installation, activation and field acceptance.
