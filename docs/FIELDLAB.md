...
Core/rules/daemon tests additionally cover the 60-second minimum-age protection, exact product/version allowlisting, controller-version fail-closed behavior, durable abandonment grace/reset, frozen-plan ordering, PREPARED-before-delivery journals, TERM-before-KILL per exact target, membership/identity change aborts, bounded revival attribution, persistence recovery/retention, exact-incident protect/unprotect, IPC controls, report-only signal suppression, and terminal receipts after post-delivery failures. The source and installed rule packs admit Chrome for Testing only at bundle identifier `com.google.chrome.for.testing` and exact versions `151.0.7922.34` or `152.0.7977.42`; the nearest wrong version plus missing, mixed, wrong-product, or controller-unverified facts remain `PROTECTED`.
...
Runtime-artifact source tests retain the dormant DAP engine's targeted `proc_listpidspath` results including stale-errno zero, open and `O_EVTONLY` references, canonical-to-quarantine rename continuity, complete argv/reference failure, unsafe ownership/type/link cases, parent/file identity changes, symlink refusal, last-hop replacement races, exclusive same-directory quarantine, and durable PREPARED/disposition recovery. Source and installed `0.4.0` packs disable DAP admission, and policy tests require zero runtime-artifact candidates. Separate synthetic storage-residue tests accept only the exact Chrome code-sign clone shape, reject symlinks/unexpected/incomplete traversal, sum logical regular-file bytes, preserve report-only fixtures, and exercise enforce-only descriptor-relative deletion of full and mixed eligible subsets plus failure rescans. Those tests authorize mutation only inside the exact temporary clone fixture root they create. Historical installed runs add controlled live-DAP points, but two DAP P2 residuals remain outside the proof: crash recovery for a canonical-to-quarantine rename, and the final same-UID pathname swap interval before `unlinkat`. No test authorizes profile, socket, PID-file or unrelated directory deletion.
...
The harness refuses ordinary Chrome and fast timings. It first proves the installed service is ready report-only, launches one unique CfT profile, admits one exact detached candidate, requires that candidate to contain no runtime artifacts, and sends SIGSTOP only to that exact root. It then arms the same ready generation, requires one journaled full-timing terminal process cleanup, and proves every signal target belongs to the pre-arm frozen field tree while every pre-existing ordinary-Chrome root preserves exact identity. The terminal receipt and durable attempt journal must both contain zero artifact actions; a separate retained proof file records those counts and whether the profile's DAP file happened to remain. Exact receipt polling is read-only and incident-scoped; it runs separately from the 250-millisecond native absence proof. Next the harness restarts launchd's same generation and requires a visible unavailable/recovery/report-only transition followed by a new daemon instance and fresh enforcement epoch, without resending or mutating the completed cleanup journal. Teardown durably disarms the same instance to report-only and retains the profile/evidence for inspection.

Passing the current harness proves one installed managed-generation **process-only** cleanup/restart transaction on one host. It proves that current rule policy did not deliver or journal an artifact action; it does not prove or accept the dormant artifact path, broader versions, controller-bearing eligibility, sleep/wake survival, sustained pressure behavior, multi-day dogfood, or release readiness.
...
Generation 13 historical result: **passed on 2026-09-02 in 378.73 seconds** under the earlier artifact-enabled packs. One unique eight-member CfT 151 tree produced eight TERM actions and one exact-root KILL, zero survivors, both revival checks, an exact DAP `removed` disposition, and `52,838,400` estimated bytes reclaimed. Same-generation restart created a fresh instance and epoch without duplicating the journal, and teardown returned the service to report-only. This proves the BGX-2 installed process/artifact/restart path at one controlled point, but it does not authorize current artifact removal.

Generation 15 historical process-only result: **passed on 2026-09-02 in 384.21 seconds** under `0.3.0` packs. One unique eight-member CfT 151 tree produced eight TERM actions and one exact KILL, zero survivors, both revival checks and `54,984,704` estimated reclaimed bytes. The analyzer admitted zero runtime-artifact candidates; terminal receipt and durable journal each contained zero artifact actions; `DevToolsActivePort` remained present; public settlement reported `artifact_outcome: not_applicable` and `overall_outcome: cleared`. Same-generation restart created a fresh instance/epoch without journal duplication, and teardown returned the service to healthy report-only. Generation 19 is installed and active under `0.4.0`, but no owner-only harness ran during its observer-repair deployment; this generation-15 result must not be relabelled as generation-19 or CfT-152 evidence.

## Open exit evidence

The original seven-session/seventy-process-class real incident remains unavailable and therefore unverified. Parent/host exit variants beyond the first clean host-exit observation, controller hang, `setsid` escape, partial exit, supervisor revival, daemon restart during cooling, live wake during cooling, sustained real memory pressure, broader versions and simultaneous ordinary-browser cases, artifact crash/swap races, chaos races, sustained overhead, and multi-day dogfood remain open. Source-level corruption recovery, event delivery tests, historical generation-9/13 results and the generation-15 process-only result narrow those risks but do not establish an ambient ordinary eligible cleanup or private-v0.1 acceptance.
