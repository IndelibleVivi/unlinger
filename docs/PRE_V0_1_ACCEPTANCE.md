...
Levels 1–4 are verified at the exact admitted point. Accepted generation 15 and the schema-v4 App are installed, and `0.3.0` process-only enforcement is active after source/CI, transactional rollback/reinstall, installed full-timing acceptance, final containment, explicit arm and a stable later sweep. This is not level 5: multi-day dogfood, an ordinary ambient eligible incident and owner private-v0.1 acceptance remain open. Levels 5–7 remain separate future owner decisions.
