...
Levels 1–4 were verified for the exact historical generation-15/schema-v4/SQLite-v6/`0.3.0` CfT-151 point. Current generation 17/schema-v5/SQLite-v7/`0.4.0` independently passed Level 3 through its generation-16 install/restart/real rollback to generation 15/v6, fresh generation-17 reinstall, and packaged v5 App/restart checks. The owner then separately accepted and armed generation 17, which remained healthy through a later sweep. It has no candidate-specific controlled signal run or ambient eligible cleanup, so generation 15's Level-4 receipt cannot be borrowed as generation-17 or CfT-152 acceptance. Multi-day dogfood, an ordinary ambient eligible incident and owner private-v0.1 acceptance remain open. Levels 5–7 remain separate future owner decisions.
