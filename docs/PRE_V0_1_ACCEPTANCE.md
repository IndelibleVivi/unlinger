...
Levels 1–4 were verified for the exact installed generation-15/schema-v4/SQLite-v6/`0.3.0` point. Accepted generation 15 and its schema-v4 App are installed, and process-only enforcement is active after source/CI, transactional rollback/reinstall, installed full-timing acceptance, final containment, explicit arm and a stable later sweep. The newer schema-v5/SQLite-v7/`0.4.0` source tranche must earn each runtime level independently and currently has no installed or field claim. Generation 15 is not level 5: multi-day dogfood, an ordinary ambient eligible incident and owner private-v0.1 acceptance remain open. Levels 5–7 remain separate future owner decisions.
...
The already-exercised generation-12/15 transactions migrated stores to SQLite schema v6 while their retained prior generations understood older schemas. Their acceptance-scoped leases retained the prior manifest, report-only plist and SQLite snapshot after candidate readiness, blocked install/uninstall/mode changes until explicit accept or rollback, and provided an exact `restart-report-only` acceptance path.
...
The source SQLite-v7/schema-v5 tranche has not entered that lane. Its future Level-3 gate must retain the exact generation-15 SQLite-v6 snapshot, prove a real rollback in which the old generation reopens v6 healthy and report-only, then freshly reinstall the v7 candidate and repeat packaged schema-v5 App/restart acceptance. Existing generation-15 evidence cannot substitute for that transaction.
