...
| APP-01 | trustworthy private native client | strict DTOs, cancellable phase-aware transport, pre-send durable journal, no-resend reconciliation, separate snapshot/history presentation mappers, exact snapshot coherence, product/version-aware browser phases/session/coverage/settlement/detail projection, incident-grouped history, collapsed repeated observations, named safety checks, bilingual VoiceOver copy, direct AppKit process lifetime, one 340×420 content contract across AppKit hosts, explicit popover Back and open-current-route action | packaged notification observation; external menu-host placement remains environment-owned | schema-v4 App from UI head `267e3f3` installed; 75-test/16-suite gate and exact-head CI green; fresh installed Chinese rendered QA covers overview/history/detail |
...
| TEST-01 | executable pre-v0.1 contract | Rust migration/policy/IPC/service-lease fixtures; Swift transport/state/browser-projection/history/detail/notification/settings/router/AppKit-host tests; explicit v4/v3 fixture bundle gate; isolated and installed report-only live-socket checks | owner-only enforcement/release gates remain separate | current source gate: Rust 287 passed/2 owner-only ignored; Swift 75/16 suites; isolated v4 smoke 7 + restart 7 green; UI head `267e3f3` exact-head macOS run `33636283051` green |
