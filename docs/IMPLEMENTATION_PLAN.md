...
| RUN-02 | frozen TERM→rescan→exact KILL→postscan→revival | exact identity revalidation, PREPARED-before-signal journal, delivery-unknown recovery, owned-child tests and historical generation-9/13 full-timing points; process-only managed verifier requires zero artifact actions | process-only installed field rerun, ambient real eligible incident, broader families and chaos | exact source mechanics + historical controlled points; current install/ambient open |
| RUN-03 | remove only admitted low-risk artifact | exact DAP engine, targeted pathname-reference + complete argv proof, quarantine and durable artifact journal remain tested but dormant; every `0.3.0` pack disables admission | crash-after-quarantine and final same-UID swap P2s; sockets/PID files; explicit future re-enable decision | implemented capability, disabled by current policy |
| STORE-01 | bounded redacted durable authority | SQLite v6 migration; stable public event/recovery tokens; atomic observation batch; durable policy revision; namespace-aware typed mutation receipts with 14-day proof window | max-retention performance | installed generation 13 uses v6; focused migration/atomicity tests |
| IPC-01 | local single-shot operator + App protocols | schema v1 operator/lifecycle preserved; schema v4 strict App surface with atomic browser overview; v3 transition responses preserved; v2 typed rejection; bounded 0600 socket/eight workers; shared action policy | broader dogfood observation | source and installed v4/v3 verified; installed generation 13 is report-only |
| APP-01 | trustworthy private native client | strict DTOs, cancellable phase-aware transport, pre-send durable journal, no-resend reconciliation, one pure browser overview mapper, exact snapshot coherence, browser phases/session/coverage/settlement/detail projection, bilingual VoiceOver copy, direct AppKit process lifetime, AppKit-owned status popover/window, explicit popover Back and open-current-route action | final owner visual/packaged-notification observation; external menu-host placement remains environment-owned | schema-v4 App installed; 73-test/15-suite BGX-2 gate green; prior rendered fixture QA remains historical |
...
| BGX-02 | daemon-owned atomic browser truth and generated compatibility support | schema-v4 contract, v3 transition endpoint, atomic overview, typed compatibility, rule-generated catalog, v4 App and CLI projection | broader dogfood observation | source-complete, exact-head verified and installed as generation 13 |
| ENF-01 | current process-only enforcement policy | `0.3.0` packs preserve exact CfT 151 process admission while producing no artifact candidate; managed verifier requires zero artifact receipt/journal rows | full source/CI gate, transactional install→restart→rollback→reinstall→accept, installed full-timing run and final arm | focused source tests green; operational gate pending |
...
6. **Contracts/support — implemented:** active v4 plus transitional v3 docs/fixtures, acceptance levels, source/evidence support matrix, and separate volatile installed truth.
...
10. **Process-only activation — owner-authorized and in progress:** finish source/CI, install candidate A report-only, exact restart, execute rollback to generation 13, reinstall candidate B, repeat readiness/restart, accept, run the full-timing managed process-only harness with zero artifact actions, return report-only, then arm only after final stable readback.
11. **BGX-3 owner action — decision-gated:** do not implement a misleading report-only action until the owner chooses whether it is unavailable or records observe-only intent. Either choice must retain the installed report-only floor and requires its own acceptance evidence.
12. **BGX-4 distribution — future lane:** signing, notarization, packaging, updating, licensing and publication remain distinct owner-controlled gates after product/field acceptance.
...
- **Artifact capability remains DAP-only, but current admission is none.** The `0.3.0` packs suppress every artifact candidate while preserving the dormant implementation and both P2 residuals for a separate decision.
- **The installed claim is narrow.** Generation 13 is accepted and report-only, but its own lease was not executed before acceptance. The process-only replacement must perform the full rollback/reinstall transaction; multi-day observation and release decisions remain separate gates.
...
Owner-only live CfT harnesses remain ignored. The installed generation-13 report-only and historical controlled enforcement evidence permit **private enforcement candidate for the exact admitted point**, but the current process-only source still requires the full local/CI and transactional installed gates before activation. Multi-day dogfood, ambient real eligibility, artifact re-enable, Intel/universal, signing/notarization/distribution and public alpha remain separate gates.
