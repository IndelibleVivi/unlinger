...
| DET-02 | 6.2 | every hard gate; score never authorizes cleanup | gate truth test, two observations, durable grace, exact revalidation | live false-positive and race evidence | implemented |
...
| RUN-02 | 7.3 | TERM, rescan, exact KILL, post-scan, bounded revival | frozen executor, synthetic ordering/race tests, owned-child macOS signal test | isolated real framework trees and full chaos matrix | implemented, not activated |
...
| STORE-01 | 8.4 | bounded SQLite receipts and local read-mostly IPC | schema migration guard, redacted typed events, 14 d/10k prune, durable cooling/pause, 0600 socket round-trip | corruption recovery and long-running load evidence | implemented |
| RULE-01 | 8.5 | embedded versioned packs with positive and nearest counterexamples | three validated embedded TOML packs and eight-case corpus | exact supported-version field evidence | implemented, observational versions |
...
| SAFE-01..13 | 11 | listed safety invariants | source guards, synthetic counterexamples, exact owned-child test, failure receipts | artifact rules, chaos completion, dogfood/public-alpha zero-FP evidence | partial |
| ACC-01 | 12 | latency, safety, completeness, overhead targets | one 28 ms live read-only snapshot only | sustained benchmark, cleanup latency/completeness, dogfood and alpha | planned |
| TEST-01 | 13.1-13.4 | fixture, live, adversarial, comparative evidence | 32 Rust tests plus report-only daemon/CLI smoke | real corpus, remaining chaos, benchmarks, comparative Field Lab | in progress |
...
2. **P1 Deterministic cleanup — source candidate implemented:** durable cooling → frozen plan → exact revalidation → TERM/rescan/exact KILL → post-scan/revival → terminal receipt. Isolated framework/live chaos acceptance remains open.
3. **P2 Ambient daemon — partially implemented:** periodic scheduler, SQLite, IPC, daemon, and CLI exist; event sources, LaunchAgent packaging, activation, and dogfood do not.
