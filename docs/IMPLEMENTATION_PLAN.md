...
| TEST-01 | executable pre-v0.1 contract | Rust migration/policy/IPC/service-lease fixtures; Swift transport/state/browser-projection/history/detail/notification/settings/router/AppKit-host tests; explicit v4/v3 fixture bundle gate; isolated and installed report-only live-socket checks | owner-only enforcement/release gates remain separate | current source gate: Rust 287 passed/2 owner-only ignored; Swift 75/16 suites; isolated v4 smoke 7 + restart 7 green; exact-head macOS CI pending for this UI revision |
