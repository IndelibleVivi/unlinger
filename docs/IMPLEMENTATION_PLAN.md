...
| TEST-01 | executable pre-v0.1 contract | Rust migration/policy/IPC/service-lease fixtures; Swift transport/state/browser-projection/detail/notification/settings/router/AppKit-host tests; active v3 fixture bundle gate; isolated and installed report-only live-socket checks | owner-only enforcement/release gates remain separate | current source gate verified: Rust 276 passed/2 owner-only ignored; Swift 96/15 suites; isolated smoke 6 + restart 6 green; BGX-1 exact-head CI run `33596733773` green |
