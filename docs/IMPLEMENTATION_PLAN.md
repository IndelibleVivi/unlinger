...
| RESIDUE-01 | identify exact Chrome code-sign clone residue without unsafe deletion | bounded exact-root scanner, typed latest observation, logical-size/APFS caveat, v5 observe-only UI; no path persistence and no cleanup authority | complete live-reference proof and a separate owner decision before any cleanup path | source synthetic gates plus generation-17 clear/absent observation passed; no live deletion path |
| ENF-01 | current process-only enforcement policy | source/installed `0.4.0` admits exact CfT 151 and 152 while producing no artifact candidate; managed verifier requires zero artifact receipt/journal rows | generation-17/CfT-152 field evidence, multi-day dogfood and ambient ordinary eligible incident | allowlist fixture-verified; generation 17 active after Level-3 transaction, but only generation 15 retains a controlled process-only point |
...
12. **Impact + residue installed lane — Level 3 complete, privately active:** generation 16 installed/restarted report-only, passed v5 App checks and really rolled back to generation 15/v6; generation 17 freshly reinstalled, repeated v5 App/restart checks, was accepted, explicitly armed and stayed healthy through a later sweep. This is not a generation-17 controlled signal or CfT-152 field result.
...
- **The installed claim is narrow.** Generation 17 is active process-only after its own full rollback/reinstall/App lane but no candidate-specific controlled signal. Generation 15's CfT-151 field point remains historical evidence. Multi-day observation, an ambient ordinary eligible incident and release decisions remain separate gates.
