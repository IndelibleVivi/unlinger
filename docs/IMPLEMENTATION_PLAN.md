...
| TEST-01 | executable pre-v0.1 contract | Rust migration/policy/IPC/service-lease fixtures; Swift transport/state/detail/notification/settings/router/AppKit-host tests; active v3 fixture bundle gate; isolated and installed report-only live-socket checks | owner-only enforcement/release gates remain separate | full gate verified: Rust 276 passed/2 owner-only ignored; Swift 66/14 suites; isolated smoke 6 + restart 6 and installed 6 + restart 6 green; rollback-hardening exact-head CI green |
