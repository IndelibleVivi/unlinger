...
| STORE-01 | bounded redacted durable authority | SQLite v6 migration; stable public event/recovery tokens; atomic observation batch; durable policy revision; namespace-aware typed mutation receipts with 14-day proof window | max-retention performance | installed generation 15 uses v6; focused migration/atomicity tests |
| IPC-01 | local single-shot operator + App protocols | schema v1 operator/lifecycle preserved; schema v4 strict App surface with atomic browser overview; v3 transition responses preserved; v2 typed rejection; bounded 0600 socket/eight workers; shared action policy | broader dogfood observation | source and installed v4/v3 verified; generation 15 active process-only |
| APP-01 | trustworthy private native client | strict DTOs, cancellable phase-aware transport, pre-send durable journal, no-resend reconciliation, stored snapshot/history presentation rebuilt once per state transition and published only on value change, exact snapshot coherence, product/version-aware browser phases/session/coverage/settlement/detail projection, incident-grouped history, collapsed repeated observations, stable SwiftUI identity, one outer Accessibility element per history link, bilingual VoiceOver copy, transient popover root lifetime, one 340×420 content contract across hosts | multi-day App dogfood; packaged notification observation; external menu-host placement remains environment-owned | source repair and 462-probe fixture gate verified; repaired bundle installed byte-for-byte; live-history acceptance passed 600 RSS samples and 1,398 successful Accessibility reads; ordinary App active |
...
| TEST-01 | executable pre-v0.1 contract | Rust migration/policy/IPC/service-lease fixtures; Swift transport/state/browser-projection/history/detail/notification/settings/router/AppKit-host tests; 50-incident fixture plus real Accessibility-tree external-RSS gate; explicit v4/v3 fixture bundle gate; isolated and installed report-only live-socket checks | new installed stress repetitions and enforcement/release gates remain owner-only | Rust 287 passed/2 owner-only ignored; Swift 78/16 suites; isolated v4 smoke 7 + restart 7; 120-second native AX fixture gate 462/0; ten-minute installed live-history gate 600 RSS samples + 1,398 successful full-tree reads |
...
| ENF-01 | current process-only enforcement policy | `0.3.0` packs preserve exact CfT 151 process admission while producing no artifact candidate; managed verifier requires zero artifact receipt/journal rows | multi-day dogfood and ambient ordinary eligible incident | source/CI, rollback/reinstall, generation-15 full-timing run, final arm and later sweep passed; active level 4 |
