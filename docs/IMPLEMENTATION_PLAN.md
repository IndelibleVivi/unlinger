...
| APP-01 | trustworthy private native client | strict DTOs, cancellable phase-aware transport, pre-send durable journal, no-resend reconciliation, coalesced refresh, typed detail, local diagnostics, stable row identity, bilingual UI, AppKit-owned status popover/window, explicit popover Back and open-current-route action | final installed navigation/window visual recheck | installed App + 66-test/14-suite gate + bundle and ordinary-window interaction verified |
...
| TEST-01 | executable pre-v0.1 contract | Rust migration/policy/IPC/service-lease fixtures; Swift transport/state/detail/notification/settings/router/AppKit-host tests; active v3 fixture bundle gate; isolated and installed report-only live-socket checks | owner-only enforcement/release gates remain separate | prior full gate verified: Rust 272 passed/2 owner-only ignored; current Swift 66/14 suites; isolated smoke 6 + restart 6 and installed 6 + restart 6 green; current UI exact-head CI pending |
