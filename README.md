...
The current allowed daemon claim is **private enforcement candidate for the exact admitted point**. Accepted generation 15 remains installed and actively enforces the process-only `0.3.0` policy, which preserves exact TERM/KILL authorization while disabling every runtime-artifact policy. After a second severe browser-history/Accessibility memory runaway, source removed the shared-navigation and repeated-history-projection feedback surfaces and passed its fixture-only hard-cutoff gate. The repaired schema-v4 ad-hoc-signed native App was then owner-authorized for replacement and a ten-minute installed field acceptance against the live generation-15 history route: 600 external RSS samples stayed below 40 MiB peak and 1,398 successful full-tree Accessibility reads retained one stable bounded history structure. A later bounded menu-bar repair enabled the macOS system permission, replaced the blank custom icon path with a system symbol, assigned a stable status-item identity, and verified the real popover before and after restart under Thaw 2.0.1-rc.1 on the built-in display. The repaired App now runs in ordinary menu-bar mode; this result is not multi-day App dogfood, packaged-notification proof or universal external-menu-host/display evidence. The daemon installation followed exact-head CI, candidate-A restart and real rollback to generation 13, candidate-B reinstall/restart/accept, a full-timing managed field pass, final report-only containment, explicit arm, and a stable later sweep.

The newer schema-v5/SQLite-v7/`0.4.0` source tranche is source-only. It makes cleanup work visible through independent impact authority, compresses repeated observations into spans, adds exact CfT `152.0.7977.42` source eligibility, and observes Chrome code-sign clone residue without deleting it. It has not replaced the installed daemon or App and has no borrowed activation or field-acceptance claim.

Field evidence remains narrower than the source surface. The generation-15 managed full-timing run reclaimed one isolated Chrome-for-Testing `151.0.7922.34` tree: eight exact processes, nine ordered signal actions, zero survivors, two revival checks, same-generation fresh-epoch restart without journal duplication, and final harness containment. It admitted zero artifact candidates, wrote zero receipt/journal artifact actions, left `DevToolsActivePort` present, and projected `artifact_outcome: not_applicable`. After activation, a later complete sweep remained healthy and protected all four observed CfT `152.0.7977.42` sessions as unsupported under installed `0.3.0`; source `0.4.0` now admits that exact version but has no controlled or ambient cleanup evidence for it. Historical generation-9/13 DAP results do not authorize either runtime to delete artifacts; both DAP P2s remain dormant. Multi-day dogfood, an ambient real eligible incident, broader version evidence, Intel/universal, signing/notarization/distribution and public alpha remain open.
