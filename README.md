...
- **Inspect and safely clean one exact disk-residue family.** Chrome code-sign clone count and logical size are visible. The source daemon leaves them unchanged in its default report-only mode; valid enforce mode evaluates each exact candidate independently and removes only candidates that were unchanged across two observations and have no executable, absolute-argument or matching cleanup-helper reference. A live candidate stays protected without blocking stable unreferenced siblings. Ordinary Chrome and Chrome Helpers running outside candidate directories do not block stale-clone cleanup. Current source durably records every attempted removal as a path-free outcome with aggregate counts, reports an interrupted attempt as delivery-unknown instead of inferring success from a later smaller count, and presents that result in the source App. Profiles, browser data and runtime artifacts remain outside this path. The reference installation has durable clone-result authority; an earlier generation supplied one bounded field result: with ordinary Chrome continuously open, eight stable stale clones were removed and its one live clone remained protected. This is not multi-day evidence, and logical bytes are not guaranteed physical APFS reclaim.
...
| Optional existing-session integration | Ordinary Playwright CLI from `playwright-core` `1.62.1`, only with an exact host owner lease; source primitive and tests exist, automatic host adapter and field evidence do not |
...
| How can a host provide exact lifetime evidence? | [Operator IPC](docs/IPC.md#optional-host-session-owner-commands) |
