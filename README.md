...
The installed private deployment is accepted generation 17, serving frontend schemas v5/v4/v3 over SQLite v7. A check on 2026-09-08 found it in terminal `Failed` after a SQLite disk I/O error. Owner-authorized exact-generation recovery and explicit re-arm restored healthy `ReadyEnforce`; the original I/O cause remains unknown. The observer fix for real Chrome `.app.bundle` clones is source-verified but not installed yet. See [`docs/current-state.md`](docs/current-state.md) for the current diagnosis. Its deployment used the exact source candidate after green local and exact-head CI gates: candidate A generation 16 installed and restarted report-only, then executed a real rollback in which the generation-15 CLI/daemon reopened the restored SQLite-v6 store healthy; the same candidate freshly reinstalled as generation 17, repeated packaged schema-v5 App/live-socket and daemon-restart checks, was accepted, explicitly armed, and remained healthy through a later ambient sweep. The canonical Applications location now contains only the schema-v5 App; its old schema-v4 bundle was removed to the owner's Trash after acceptance.
...
Do not improvise an installed migration or bypass the service CLI. [`docs/INSTALLED_DOGFOOD.md`](docs/INSTALLED_DOGFOOD.md) records the completed report-only candidate/rollback lane; [`docs/current-state.md`](docs/current-state.md) owns current runtime truth. Generation 17 is accepted with no pending candidate rollback lease and is healthy `ReadyEnforce` after the 2026-09-08 recovery. Use the exact active-generation CLI for status and require a new owner decision before install, uninstall or another mode change.
