...
- a SwiftPM native menu-bar client under `apps/UnlingerApp`, with a direct AppKit `@main`, process-lifetime delegate ownership, an AppKit-owned status item/popover and reusable ordinary window hosting the SwiftUI surface, explicit back/window routing, strict v3 DTOs, bilingual copy, canonical-fixture previews/tests, crash-durable pre-send mutation journal, stale-safe refresh/detail state, capability-gated ordinary actions, bounded deduplicated local notifications, menu-client-only launch at login, and a private ad-hoc-signed `.app` bundler.
