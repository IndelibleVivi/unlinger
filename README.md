...
The current allowed claim is **private enforcement candidate for the exact admitted point**. Accepted generation 15 and the matching schema-v4 ad-hoc-signed native App are installed for private dogfood. Process-only enforcement is active: its `0.3.0` packs preserve exact TERM/KILL authorization while setting every runtime-artifact policy to false. Installation followed exact-head CI, candidate-A restart and real rollback to generation 13, candidate-B reinstall/restart/accept, a full-timing managed field pass, final report-only containment, explicit arm, and a stable later sweep.
