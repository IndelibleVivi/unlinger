...
fn project_payload(
    ...
    Ok(match payload {
        ...
        | IpcPayload::TaskLease(_)
        | IpcPayload::TaskStatus(_)
        | IpcPayload::SessionOwnerLease(_)
        | IpcPayload::SessionOwnerStatus(_) => {
            return Err(ControlError::Unavailable(
                "operator ownership/lifecycle responses are not part of frontend IPC".to_owned(),
            ));
        }
    })
}
