...
- The installed native App remains intentionally stopped after the owner observed a second severe browser-history/Accessibility memory runaway. Source uses independent menu/window routers, one-way route handoff, and stored browser/history presentation. Do not reinstall, relaunch, or run installed-App Accessibility stress without a new applicable owner decision; fixture-only source validation is the current frontend lane.
