...
Source cache-maintenance work adds SQLite v12, `store/tool_cache.rs` latest-attempt
transactions, the bounded npm native adapter in `tool_cache.rs` / `tool_cache_driver.js`,
and an optional frontend-v5 `tool_cache_maintenance` projection. The first family
is default npm download content only (npm 11.19.0 / cacache 20.0.4), with weekly
native upkeep, PREPARED before spawn, cancellable owned child, no raw output/path
persistence, and independent native accounting. No runtime-artifact flag is
enabled. Source checks use test-owned cache roots only; never verify/prune a live
cache as a smoke test. Source v12 does not change installed generation 36 / v11;
all installed and historical evidence below retains its dated scope. New source
maintenance is not installation or field acceptance. `docs/current-state.md`
owns progress and exact verification.
...
current App passes 101 Swift tests and release bundling. The installed App's
