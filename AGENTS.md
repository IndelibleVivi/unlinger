...
complete native Chrome/Chrome-Helper absence and descriptor-relative no-follow
deletion followed by immediate rescan. This source path is not installed or
field-verified; accepted generation 25 remains observe-only for clone residue.
...
- `crates/unlinger-macos`: macOS `libproc`/`sysctl` snapshots, transient app-version facts, exact Playwright package/registry/socket session discovery, native process-exit/wake/pressure sources, exact-identity signal adapter, exact runtime-artifact adapter, and the canonical exact-root Chrome clone scanner/cleanup state machine. The clone path retains one prior candidate identity set in memory, reuses the native snapshot to require complete absence of bundle-confirmed Chrome/Chrome Helper processes, and performs no-follow descriptor-relative deletion only after two stable observations. A PID confirmed as `SZOMB` through `KERN_PROC_PID` is gone for live-identity purposes rather than an unreadable live process. `/bin/ps`, process-name kills, and broad PGID kills are not production paths.
