...
ordinary CLI `1.62.1`; other packs remain `0.4.0`. It is source-only. All
generation-23 / SQLite-v8 / Playwright-0.5.0 / installed-App statements below
refer to the unchanged reference installation. The candidate preserves frontend
...
source tests or old field acceptance as
candidate installation, activation, ambient or multi-day acceptance, and do not
open a migrated v10 DB with an old binary. `docs/current-state.md` owns the exact
candidate boundary.
...
- Installed generation 23 and the existing native App use frontend schema v5 with strict v4 and v3 compatibility endpoints, operator schema v1, and SQLite v8. The App emits v5 only and requires the daemon-owned atomic `browser_overview`, including impact/residue and observation-span facts. Frontend schemas expose only `unlinger-protocol` ordinary commands/public DTOs and contain no lifecycle command. Schema v2 is historical and must receive typed `unsupported_schema`; never silently downgrade the App to v4, v3 or v1.
...
- Task lifetime is defined by `docs/TASKS.md`: source SQLite v10 (reference installation v8), gated actual command owner, schema-v1 local peer/capability commands, immutable release and exact controller bindings. Source Playwright pack `0.6.0` retains task-owned CLI `1.63.0-alpha-2026-08-31`; the installed pack remains `0.5.0`, and the other packs stay `0.4.0`. Task release must never arm, bypass cooling/protection, synthesize impact or adopt old/unrelated sessions.
