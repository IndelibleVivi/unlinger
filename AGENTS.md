...
- Source task lifetime is defined by `docs/TASKS.md`: SQLite v8, gated actual command owner, schema-v1 local peer/capability commands, immutable release and exact controller bindings. Playwright pack `0.5.0` verifies task-owned CLI `1.63.0-alpha-2026-08-31`; the other packs stay `0.4.0`. This source capability is separate from generation 19's installed `0.4.0` policy. Task release must never arm, bypass cooling/protection, synthesize impact or adopt old/unrelated sessions.
