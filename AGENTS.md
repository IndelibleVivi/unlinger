...
accepted generation 30 with SQLite v10 and the aggregate candidate-reference
clone policy. Generation 29 completed restart checks and a real rollback to
generation 27 before the same exact artifact was freshly installed, accepted
and armed as generation 30; it is currently healthy `ReadyEnforce`. The
...
live candidate does not block stable unreferenced siblings. This per-candidate
source gate is not installed or field-verified. Current App source,
...
The ordinary daemon default remains report-only. Accepted installed generation 30 contains Playwright `0.6.0`, other packs `0.4.0`, SQLite v10, mapped-vnode executable identity, the transaction-rollback recovery correction and the aggregate candidate-reference clone gate. Generation 29 installed at the report-only floor, passed App/socket checks and restart, actually rolled back to healthy generation 27, and the same exact artifact was freshly installed, accepted and armed as generation 30. It is currently healthy `ReadyEnforce`. The installed schema-v5 App is `c17e60f`. Current source additionally refines clone cleanup to per-candidate stability/reference/removal, but that refinement is not yet installed. `docs/current-state.md` owns the exact evidence. Historical generation-15/17/19/23/25 results must not be borrowed as generation-30 cleanup, ambient or multi-day acceptance. The original generation-17 SQLite-I/O failure cause remains unknown.
