...
- The installed native App was owner-authorized for replacement and bounded field acceptance on 2026-09-04. The installed bundle is byte-identical to the ad-hoc-signed candidate built from source-truth head `322da05` (memory-repair implementation `e7194f1`), and ordinary normal-mode App use is active. Its real generation-15 browser-history acceptance held the live history route for ten minutes, sampled RSS 600 times, and completed 1,398 successful trusted full-tree Accessibility reads: RSS was 18,240 KiB initially, 31,424 KiB finally and 40,736 KiB maximum, with 13,184 KiB retained growth under the 384 MiB absolute / 96 MiB growth cutoffs. This accepts the repaired App only for that bounded condition, not multi-day App dogfood, packaged notification delivery or external menu-host placement. Do not replace the App or repeat installed-App stress without a new applicable owner decision.
