...
accepted generation 27 with SQLite v10 and the earlier global-Chrome blocking
clone policy; an interrupted generation-28 candidate transaction was
transactionally rolled back and generation 27 is currently healthy
`ReadyReportOnly`. The canonical schema-v5 App is the installed `c17e60f`
memory-repair build.
Current source after that installed baseline adds owner-approved automatic cleanup
for the exact Chrome `code_sign_clone` storage family: report-only still never
mutates; valid enforce requires two consecutive 15-minute stable observations,
complete native proof that no process references a candidate by executable or
absolute argv path and no clone-cleanup helper names its suffix, plus
descriptor-relative no-follow deletion followed by immediate rescan. Ordinary
Chrome/Chrome-Helper processes outside the candidate set do not block. This
corrected source gate is not installed or field-verified. Current App source,
...
every existing signal gate. Full daemon source gates and the `c17e60f`
exact-head CI pass; the current App passes 91 Swift tests, a 460-probe
Accessibility/RSS lane, a 900-sample real-daemon packaged guard and a
2,400-sample installed-App guard with a 22,192 KiB maximum. Do not treat
source tests or old field acceptance as ambient or multi-day acceptance, and do
not open a migrated v10 DB with an old binary. `docs/current-state.md` owns the
exact installed and activation boundary.

## Current hard boundary

The ordinary daemon default remains report-only. Accepted installed generation 27 contains Playwright `0.6.0`, other packs `0.4.0`, SQLite v10 and the earlier global-Chrome blocking clone cleanup gate. It was restored transactionally after generation 28 failed its first-scan readiness gate and is currently healthy `ReadyReportOnly`, not armed. The installed schema-v5 App is `c17e60f`. Generation 26 installed at the report-only floor and actually rolled back to healthy generation 25 before fresh generation 27 was accepted and armed; the later generation-28 attempt restored generation 27 and deliberately left the service at the report-only floor. Current source includes the `516d483` candidate-level clone gate plus mapped-vnode snapshot and rollback-recovery corrections, but is not yet installed. `docs/current-state.md` owns the exact evidence. Historical generation-15/17/19/23/25 results must not be borrowed as generation-27 cleanup, ambient or multi-day acceptance. The original generation-17 SQLite-I/O failure cause remains unknown.
...
- The owner observed a second installed-App runaway to about 19 GB on 2026-09-16. A system CPU-resource report captured growth from 42 MiB to about 2.0 GiB in 138 seconds on the main SwiftUI/AttributeGraph thread, dominated by `AppKitPopUpAdaptor` item Accessibility attributed-text updates; the owner then Force Quit that exact App with SIGTERM. The old App is not credited with later low-memory samples. The recoverably installed `c17e60f` App removes that popup adaptor from the three affected controls and adds equal-publication/closed-window regressions; its exact installed process passed 2,400 one-second RSS samples with a 22,192 KiB maximum and remained running. This is bounded installed evidence, not a uniquely reproduced trigger or multi-day acceptance. Existing rollback Apps must remain intact.
...
- `crates/unlinger-macos`: macOS `libproc`/`sysctl` snapshots, transient app-version facts, exact Playwright package/registry/socket session discovery, native process-exit/wake/pressure sources, exact-identity signal adapter, exact runtime-artifact adapter, and the canonical exact-root Chrome clone scanner/cleanup state machine. Executable dev/inode identity comes from the process's already-mapped `PROC_PIDREGIONPATHINFO` vnode and must match `pidpath`; production snapshotting must not reopen arbitrary executable pathnames. The clone path retains one prior candidate identity set in memory, reuses the native snapshot to reject exact matching cleanup-helper suffixes and executable/absolute-argv paths inside a candidate, and performs no-follow descriptor-relative deletion only after two stable observations. Ordinary bundle-confirmed Chrome/Chrome Helper processes outside candidates are not references; incomplete coverage or insufficient Chrome-looking facts fail closed. A PID confirmed as `SZOMB` through `KERN_PROC_PID` is gone for live-identity purposes rather than an unreadable live process. `/bin/ps`, process-name kills, and broad PGID kills are not production paths.
- `crates/unlinger-daemon`: durable cooling grace, native-hint/periodic scheduler, SQLite v10 optional session ownership plus v9 action-attribution repair plus v8 task ownership, observation spans, independent impact/action/mutation authority, stable public event/receipt authority, one atomic schema-v5 browser overview projection with v4 compatibility, typed storage-residue persistence and enforce/report-only lifecycle gate, shared public-action policy, retention, local IPC, managed lifecycle state, and reconciliation engine.
