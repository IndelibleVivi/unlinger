...
the native supervisor and exact-child cancellation; `main.rs` owns one background
cache worker, so process observation continues during maintenance. Exact process
...
Source v13 does not change installed generation 36 / v11. Any replacement needs
...
installed, accepted and armed as generation 36; it is currently healthy
`ReadyEnforce` with no rollback lease. Earlier generation 34, with ordinary
