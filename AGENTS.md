...
The ordinary daemon default remains report-only. Accepted installed generation 17 was activated with the owner-approved process-only `0.4.0` policy after exact-head CI, a generation-16 install/restart/real rollback to generation 15 and SQLite v6, a fresh generation-17 reinstall/restart/schema-v5 App check, explicit acceptance and arm, and a stable later sweep. The 2026-09-08 check found terminal `Failed` after a recorded SQLite disk I/O error. The owner then authorized recovery: the exact service CLI replaced that failed instance with a healthy, quiescent report-only instance, followed by explicit re-arm and a later healthy `ReadyEnforce` readback. The original I/O cause remains unknown. Generation 17 has installed-integration evidence but no candidate-specific full-timing signal run; the generation-15 CfT-151 Level-4 point remains historical evidence and must not be borrowed as generation-17/CfT-152 field acceptance.
...
- Do not run a second `unlingerd --enforce`, uninstall the service, replace generation 17, widen eligibility, re-enable artifacts, or change the installed mode without a new applicable owner decision. Preserve the active generation-17 service while doing source-only work.
...
- Generation 17 is accepted, healthy `ReadyEnforce` after owner-authorized exact-instance recovery, generation/epoch-bound, and has no rollback lease. A successful scan alone must never rehabilitate a terminal Failed instance. Candidate A generation 16 passed exact report-only restart and real rollback to healthy generation 15 with the old CLI/daemon reopening restored SQLite v6; candidate B reinstalled as generation 17, repeated readiness/restart/schema-v5 App checks, and was accepted before the final arm and later sweep. Any future replacement must perform its own equivalent transaction. Rollback persists `RollbackInProgress` before physical mutation and must replay only transaction-owned candidate/prior selection states until the prior report-only generation is healthy.
