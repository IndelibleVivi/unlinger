...
clone policy. The canonical schema-v5 App is still the `628d822` build.
...
also replaces the diagnosed SwiftUI popup Accessibility adaptor with one
idempotent native `NSPopUpButton` path, suppresses equal-state publications and
releases the ordinary-window host on close; it is not yet the installed App.
...
every existing signal gate. Full daemon source gates and exact-head CI pass; the
current App passes 91 Swift tests, a 460-probe Accessibility/RSS lane and a
900-sample real-daemon packaged guard below 96 MiB RSS. Do not treat
...
The ordinary daemon default remains report-only. Accepted installed generation 27 runs the owner-authorized process-only policy with Playwright `0.6.0`, other packs `0.4.0`, SQLite v10 and the earlier global-Chrome blocking clone cleanup gate. The installed schema-v5 App remains `628d822`. Generation 26 installed at the report-only floor and actually rolled back to healthy generation 25 before fresh generation 27 was accepted and armed. Current source `516d483` narrows only the clone reference gate and is not installed. `docs/current-state.md` owns the exact evidence. Historical generation-15/17/19/23/25 results must not be borrowed as generation-27 cleanup, ambient or multi-day acceptance. The original generation-17 SQLite-I/O failure cause remains unknown.
...
- `crates/unlinger-cli`: gated `task run`/`task status`, coherent optional host-adapter `session run`/`session status`, status, atomic `browser status`, history, explain, doctor, pause/resume, retry, protect/unprotect, dry-run scan, redacted diagnostic export, and transactional LaunchAgent lifecycle.
