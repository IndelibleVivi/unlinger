...
- Task lifetime is defined by `docs/TASKS.md`: source SQLite v9 (reference installation v8), gated actual command owner, schema-v1 local peer/capability commands, immutable release and exact controller bindings. Playwright pack `0.5.0` verifies task-owned CLI `1.63.0-alpha-2026-08-31`; the other packs stay `0.4.0`. Task release must never arm, bypass cooling/protection, synthesize impact or adopt old/unrelated sessions.
...
- `crates/unlinger-daemon`: durable cooling grace, native-hint/periodic scheduler, SQLite v9 task ownership and action-attribution repair plus observation spans plus independent impact/action/mutation authority, stable public event/receipt authority, one atomic schema-v5 browser overview projection with v4 compatibility, observe-only storage-residue persistence, shared public-action policy, retention, local IPC, managed lifecycle state, and reconciliation engine.
- `crates/unlinger-cli`: gated `task run`/`task status`, status, atomic `browser status`, history, explain, doctor, pause/resume, retry, protect/unprotect, dry-run scan, redacted diagnostic export, and transactional LaunchAgent lifecycle.
