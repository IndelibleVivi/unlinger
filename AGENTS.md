...
- `crates/unlinger-macos`: macOS `libproc`/`sysctl` snapshots, transient app-version facts, exact Playwright package/registry/socket session discovery, native process-exit/wake/pressure sources, exact-identity signal adapter, and exact runtime-artifact adapter. A PID confirmed as `SZOMB` through `KERN_PROC_PID` is gone for live-identity purposes rather than an unreadable live process. `/bin/ps`, process-name kills, and broad PGID kills are not production paths.
- `crates/unlinger-daemon`: durable cooling grace, native-hint/periodic scheduler, SQLite v10 optional session ownership plus v9 action-attribution repair plus v8 task ownership, observation spans, independent impact/action/mutation authority, stable public event/receipt authority, one atomic schema-v5 browser overview projection with v4 compatibility, observe-only storage-residue persistence, shared public-action policy, retention, local IPC, managed lifecycle state, and reconciliation engine.
