...
ordinary CLI `1.62.1`; other packs remain `0.4.0`. The exact candidate is now
installed as accepted generation 25 with SQLite v10. The canonical schema-v5
App is the memory-hardened `628d822` build; it retains the generation-25 wire
contract without changing daemon policy or lifecycle.
...
corrected source gate is not installed or field-verified; `docs/current-state.md`
owns the installed boundary.
...
every existing signal gate. Full local source/App gates pass, including the
isolated report-only socket and current 457-probe Accessibility/RSS lane. Do not treat
...
The ordinary daemon default remains report-only. Accepted installed generation 25 runs the owner-authorized process-only policy from source implementation `e26297b`: Playwright `0.6.0`, other packs `0.4.0` and SQLite v10. The installed schema-v5 App is the compatible memory-hardened `628d822` build. Exact-source and current-head CI passed. Generation 24 installed/restarted at the report-only floor and actually rolled back to healthy, unarmed generation 23/SQLite v8; fresh generation 25 repeated migration, restart and App/socket checks before acceptance and explicit arm. `docs/current-state.md` owns the exact evidence. Historical generation-15/17/19/23 results must not be borrowed as generation-25 cleanup, ambient or multi-day acceptance. The original generation-17 SQLite-I/O failure cause remains unknown.
...
- The installed native App was owner-authorized for memory repair and replacement on 2026-09-16. The canonical Applications location contains the ad-hoc-signed `628d822` schema-v5 bundle; both the displaced `e26297b` App and the earlier `016ca58` App are retained as recoverable local siblings. Strict bundle/signature/plist equality, exact launch, one trusted installed Accessibility-tree read, 180 menu-only RSS samples and 120 visible-window RSS samples passed while generation 25 remained healthy and untouched. The source fixture separately passed 457 repeated Accessibility probes. These are bounded points, not multi-day or packaged-notification acceptance. Do not replace the App again, remove either rollback sibling or repeat installed-App stress without a new applicable owner decision.
