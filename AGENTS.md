...
- Installed generation 12 and its native App remain frontend schema v3 for private report-only dogfood. Current source adds schema v4 plus a strict v3 transition endpoint; the source App emits v4 only and requires the daemon-owned atomic `browser_overview`. Both frontend schemas expose only `unlinger-protocol` ordinary commands/public DTOs and contain no lifecycle command. Schema v2 is historical and must receive typed `unsupported_schema`; never silently downgrade the App to v3 or v1, and never describe source v4 as installed.
