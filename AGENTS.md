...
- `apps/UnlingerApp`: SwiftPM native menu-bar thin client. `Sources/UnlingerKit` owns strict schema-v3 transport/DTOs, owner-private persistence, durable mutation state, notifications, navigation, settings and UI projection, including the AppKit-owned status-item popover and reusable ordinary window hosts; `Sources/UnlingerApp` owns launch-mode wiring. `Tests/UnlingerAppTests` owns fixture/transport/mutation/concurrency/notification/window-host/live-socket coverage. `scripts/bundle.sh` assembles the private ad-hoc-signed app and `scripts/pre-v0.1-smoke.sh` owns the isolated report-only integration gate. `Contract/v3` is active; `Contract/v2` is historical only.
