...
- Frontend schema v2 and the first native SwiftUI client are source-complete but not installed or integrated with generation 9. V2 exposes only `unlinger-protocol` ordinary commands/public DTOs, including the bounded read-only `incidents` roster; lifecycle commands do not exist in that enum. Never silently downgrade a frontend to v1 `DaemonStatus`, whose internal PID/generation/epoch fields belong to CLI/service compatibility and diagnosis.
...
- `crates/unlinger-protocol`: frontend schema-v2 commands, public DTOs, capabilities, outcome enums, response envelopes, and canonical fixture decoders. It contains no service lifecycle command.
...
- `apps/UnlingerApp`: SwiftPM native menu-bar thin client. `Sources/UnlingerKit` owns schema-v2 transport/DTO/state/UI projection, `Sources/UnlingerApp` owns launch modes, `Tests/UnlingerAppTests` owns fixture/mapping/mutation/live-socket coverage, and `scripts/bundle.sh` assembles the private ad-hoc-signed app. `Contract` owns the frontend reader contract and Rust-roundtripped fixtures; `FRONTEND_BOUNDARY.md` records the stable implementation boundary. Dated Faye/Cove/agent working handoffs are private continuity and stay outside Git.
