...
- Frontend schema v2 is source-complete but not installed in generation 9. It exposes only `unlinger-protocol` ordinary commands/public DTOs; lifecycle commands do not exist in that enum. Never silently downgrade a frontend to v1 `DaemonStatus`, whose internal PID/generation/epoch fields belong to CLI/service compatibility and diagnosis.
...
- `apps/UnlingerApp/Contract`: frontend v2 reader contract and Rust-roundtripped canonical fixtures. `apps/UnlingerApp/HANDOFF.md` is a public-safe implementation handoff, not private continuity.
