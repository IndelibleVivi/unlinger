...
- Source SQLite schema v6 is incompatible with the installed generation-9 v5 binary. The service source now retains the prior manifest/plist/v5 database through `CandidateReadyReportOnly`, exposes explicit `accept-candidate`/`rollback-candidate`, and provides `restart-report-only` without arm. Installed work may proceed only through the owner-authorized `docs/INSTALLED_DOGFOOD.md` lane after exact-head CI. It must really restore generation 9 and prove its exact old CLI/daemon opens the v5 store before reinstalling the candidate; keep the second lease pending during initial dogfood and do not claim installed v3 before the full runbook passes.
