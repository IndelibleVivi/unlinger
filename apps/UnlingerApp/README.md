...
The existing schema-v5 ad-hoc-signed App remains installed for private dogfood and is compatible with the SQLite-v8 task-lifetime backend. The new backend passed its own real rollback/reinstall and App socket checks (7 + 7 before/after restart); no App replacement was needed. Live desktop Accessibility read the installed total as two reclaimed sessions, 16 processes and 244.8 MB estimated memory, alongside the unchanged observe-only 1.48-GB clone card. The new fixture RSS/Accessibility attempt could not acquire its target window and is not a passed stress check. Earlier repaired-App memory acceptance remains bounded historical evidence. [current-state](../../docs/current-state.md) owns exact current totals, runtime and deployment truth.
