...
The matching schema-v5 ad-hoc-signed App from implementation `e26297b` is installed and running for private dogfood against generation 25 / SQLite v10. The prior `016ca58` bundle remains a recoverable local sibling. Strict bundle/signature/plist equality and exact launch passed; generation 24 and fresh generation 25 each passed seven live-socket checks before and after a report-only restart, and the real rollback to generation 23/v8 also passed all seven. Fresh visual UI, packaged-notification and installed Accessibility/RSS acceptance were not performed. Earlier repaired-App memory and impact readbacks remain bounded historical evidence. [current-state](../../docs/current-state.md) owns exact current totals, runtime and deployment truth.

The earlier navigation-memory acceptance remains relevant: its independent guard completed 600 RSS samples at 18,240 KiB initial, 31,424 KiB final and 40,736 KiB maximum, while trusted Accessibility completed 1,398 full-tree reads over one stable bounded history structure. The status-item repair still uses the system symbol and one stable AppKit/Thaw identity. These bounded points permit ordinary App use but are not multi-day App dogfood, packaged-notification evidence or proof that every external menu host/display arrangement places the status item. Replacing the App or repeating installed stress requires a new owner decision. The installed App emits v5 only; the daemon retains v4 and v3 compatibility endpoints without downgrade. [`../../docs/INSTALLED_DOGFOOD.md`](../../docs/INSTALLED_DOGFOOD.md) records the repeatable transactional lane.
