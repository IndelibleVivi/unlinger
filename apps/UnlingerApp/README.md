...
The schema-v5 ad-hoc-signed App from memory-hardening commit `628d822` is installed and running for private dogfood against generation 25 / SQLite v10. The displaced `e26297b` bundle and earlier `016ca58` bundle remain recoverable local siblings. Strict bundle/signature/plist equality and exact launch passed; the installed App then passed 180 menu-only RSS samples, 120 visible-window RSS samples and one trusted Accessibility-tree read of the live overview. Generation 24 and fresh generation 25 had already passed seven live-socket checks before and after a report-only restart, and the real rollback to generation 23/v8 also passed all seven. Packaged-notification and multi-day App acceptance were not performed, and the owner-observed recurrent 20-GB event was not reproduced to a unique cause. [current-state](../../docs/current-state.md) owns exact current totals, runtime and deployment truth.

The earlier navigation-memory acceptance remains relevant: its independent guard completed 600 RSS samples at 18,240 KiB initial, 31,424 KiB final and 40,736 KiB maximum, while trusted Accessibility completed 1,398 full-tree reads over one stable bounded history structure. The status-item repair still uses the system symbol and one stable AppKit/Thaw identity. These bounded points permit ordinary App use but are not multi-day App dogfood, packaged-notification evidence or proof that every external menu host/display arrangement places the status item. Replacing the App again, removing either rollback sibling or repeating installed stress requires a new owner decision. The installed App emits v5 only; the daemon retains v4 and v3 compatibility endpoints without downgrade. [`../../docs/INSTALLED_DOGFOOD.md`](../../docs/INSTALLED_DOGFOOD.md) records the repeatable transactional lane.
