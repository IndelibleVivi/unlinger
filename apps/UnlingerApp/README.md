...
The strongest claim is **bounded installed memory-repair evidence plus a private report-only service baseline**. The recovered generation-27 daemon is currently report-only with no lease; one controlled process-cleanup run, one stable sweep and one 40-minute App guard are not private-v0.1 or multi-day acceptance, a signed distribution candidate or a public release.
