...
Accepted generation 17 and the schema-v5 ad-hoc-signed App are installed and active for private dogfood. Candidate A generation 16 installed/restarted report-only and really rolled back to generation 15, whose old CLI/daemon reopened restored SQLite v6; candidate B freshly reinstalled as generation 17, repeated the seven-test v5 live-socket suite before and after restart, was accepted, explicitly armed, and remained healthy through a later sweep. The canonical Applications location contains only the v5 bundle; the prior v4 App moved to the owner's Trash after acceptance. A live UI check read the home and cleanup-only empty-history routes without the prior repeated protected rows or 404, and the App stayed alive for more than five minutes with sampled RSS no higher than 29,408 KiB.
