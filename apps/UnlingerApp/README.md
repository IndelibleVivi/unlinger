...
The existing schema-v5 ad-hoc-signed App remains installed for private dogfood; the backend is now generation 19. The observer repair passed generation-18 install/restart/App checks and real rollback to generation 17/v7, then fresh generation-19 install/restart/App checks, live observation, acceptance and arm. No App replacement or installed-App stress was needed: a live UI readback displayed one Chrome clone, 1.48 GB logical size, and the APFS/observe-only caveat. The earlier repaired-App memory acceptance remains bounded historical evidence. [`../../docs/current-state.md`](../../docs/current-state.md) owns current runtime and deployment truth.
