...
Accepted generation 13 and the matching schema-v4 ad-hoc-signed App are installed for private dogfood and currently healthy report-only. The App consumes v4 and the daemon retains v3 only as a transition endpoint. The current locally source-complete `0.3.0` process-only rule candidate is not installed or activated by source validation. [`../../docs/INSTALLED_DOGFOOD.md`](../../docs/INSTALLED_DOGFOOD.md) records the transactional lane; the next candidate must create its own lease to generation 13, execute a real rollback, reinstall as a fresh generation, and repeat App/daemon restart checks before acceptance.

The strongest claim is **private enforcement candidate for the exact admitted point**, based on historical generation-13 controlled evidence. Current ambient enforcement is contained, no lease is pending, and the process-only replacement still has its own source/CI/transaction/field gates. This is not private-v0.1 acceptance, a signed distribution candidate or a public release.
